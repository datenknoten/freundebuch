...
const getCircleByIdIR: any = {"usedParamSet":{"circleExternalId":true,"userExternalId":true},"params":[{"name":"circleExternalId","required":false,"transform":{"type":"scalar"},"locs":[{"a":399,"b":415}]},{"name":"userExternalId","required":false,"transform":{"type":"scalar"},"locs":[{"a":439,"b":453}]}],"statement":"SELECT\n    c.external_id,\n    c.name,\n    c.color,\n    pc.external_id AS parent_circle_external_id,\n    c.sort_order,\n    c.created_at,\n    c.updated_at,\n    COUNT(fc.id)::int AS friend_count\nFROM friends.circles c\nINNER JOIN auth.users u ON c.user_id = u.id\nLEFT JOIN friends.circles pc ON c.parent_circle_id = pc.id\nLEFT JOIN friends.friend_circles fc ON fc.circle_id = c.id\nWHERE c.external_id = :circleExternalId\n  AND u.external_id = :userExternalId\nGROUP BY c.id, c.external_id, c.name, c.color, pc.external_id, c.sort_order, c.created_at, c.updated_at"};
...
const deleteCircleIR: any = {"usedParamSet":{"circleExternalId":true,"userExternalId":true},"params":[{"name":"circleExternalId","required":false,"transform":{"type":"scalar"},"locs":[{"a":71,"b":87}]},{"name":"userExternalId","required":false,"transform":{"type":"scalar"},"locs":[{"a":134,"b":148}]}],"statement":"DELETE FROM friends.circles c\nUSING auth.users u\nWHERE c.external_id = :circleExternalId\n  AND c.user_id = u.id\n  AND u.external_id = :userExternalId\nRETURNING c.external_id"};

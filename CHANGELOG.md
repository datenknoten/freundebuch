...
* /api/friends, /api/friends/search/*, /api/collectives and
/api/encounters return { data, pagination: { page, pageSize, totalCount,
totalPages } }; /api/notification-channels returns bare resources.
* `docker-compose.prod.yml` now requires `WEBAUTHN_RP_ID` to be
...
* clients whose sync token predates the retention window
perform one full resync instead of silently missing deletions.

Reported by Copilot on !198.
* PUT /api/users/me is removed; email is read-only
until a verified change-email flow exists.
* the `pageSize`, `sortBy`, `sortOrder` and `includeFacets`
query parameters are removed in favour of `page_size`, `sort_by`, `sort_order`
and `include_facets`. Affects `/api/friends`,
`/api/friends/search/paginated` and `/api/friends/search/faceted`; the other
list endpoints already used `page_size`. A client still sending an old name
gets the endpoint default rather than the value it asked for. Response bodies
are unchanged.
